...
class ApartmentPhotoForm extends StatefulWidget {
  final TabController? controller;
  final ApartmentModel? apartment;
  const ApartmentPhotoForm({super.key, this.controller, this.apartment});

  @override
  State<ApartmentPhotoForm> createState() => _ApartmentPhotoFormState();
}

class _ApartmentPhotoFormState extends State<ApartmentPhotoForm>
  ...
  void initState() {
    ...
    if (widget.apartment != null) {
      _uploadedImages.addAll(widget.apartment!.photos ?? []);
    }
  }
      ...
      listener: (context, state) {
        if (state.isValidating) {
          if (state.pickedImages.isEmpty && state.isPreFilled == false) {
            ...
            context.read<ApartmentFormCubit>().updateApartment();
          } else {
            context.read<ApartmentFormCubit>().createApartment();
          }
        }
      },
  ...
  void pickImages(List<File> pickedImages) async {
    ...
    if (images.length > remainingImages) {
      ...
      context.read<ApartmentFormCubit>().addImages(imagesToPick);
    } else {
      // ignore: use_build_context_synchronously
      context.read<ApartmentFormCubit>().addImages(images);
    }
  }
  ...
  Widget _buildImages(ApartmentFormState state, bool isLoadingPhotos) {
    if (isLoadingPhotos) {
      return Container(
        ...
        (widget.apartment?.photos?.isEmpty ?? true)) {
  ...
  Widget _buildSelectedImages(ApartmentFormState state) {
    return Column(
