...
function readFromCSVColumn(file, column_name) {
  ...
  const csvColumnData = data.map((row) =>
    row.split(",")[column_name].replace("\r", "")
  );
...
function readFromCSVToJson(file) {
  ...
  const jsonData = rows.map((row) => {
    ...
    for (let i = 0; i < headers.length; i++) {
      const value =
        rowValues[i] === undefined ? "" : rowValues[i].replace(/"/g, "");
      const key = headers[i].replace(/"/g, "");
      rowObj[key] = value;
    }
    return rowObj;
  });
...
const random_locations = readFromCSVToJson("data/uscities.csv");
const new_york_locations = random_locations.filter(
  (location) => location.state_name === "New York"
);
...
const marketplace_categories = readFromCSVToJson(
  "data/marketplace_categories_seed_data.csv"
);
...
function createRandomUser() {
  ...
  return {
    ...
    flatmates_gender_prefs: faker.helpers.arrayElement(
      user_flatmates_gender_prefs
    ),
...
function createRandomSublet(photos) {
  ...
  const random_location = faker.helpers.arrayElement(new_york_locations);
  ...
  return {
    ...
    location: "POINT(" + random_location.lng + " " + random_location.lat + ")",
    address: `${faker.location.streetAddress({
      useFullAddress: true,
    })}, ${faker.location.city()}, ${faker.location.state()}, ${faker.location.country()}`,
  };
}
...
async function seedRooms({ isApartment = false, itemCount = 100 }) {
  ...
  try {
    for (const sublet of sublets) {
      const { data, error } = await supabase
        .schema(supabaseSchema)
        .from(isApartment ? "apartments" : "sublets")
        .insert(sublet);
      if (error) {
        console.error(
          `Error seeding ${isApartment ? "apartments" : "sublets"}:`,
          error
        );
        console.error("Error prone sublet: ", sublet);
      } else {
        console.log(
          `${isApartment ? "Apartment" : "Sublet"} seeded successfully - ${
            sublet.id
          }`
        );
      }
...
function getAllMarketplacePhotos() {
  ...
  marketplace_photos.forEach((photo) => {
    ...
    if (!marketplace_items.includes(photo_name)) {
      throw new Error(
        `Photo name ${photo_name} not found in marketplace_items`
      );
    }
    marketplace_photos_map[photo_name] = readFromTxt(
      `${marketplace_photos_folder}/${photo}`
    );
  });
...
function createRandomMarketplace(photos) {
  const title = `${faker.commerce.productAdjective()} ${titleCase(
    faker.helpers.arrayElement(marketplace_items)
  )}`;
  const description = faker.commerce.productDescription();
  const location = faker.helpers.arrayElement(new_york_locations);
  const current_time = faker.date.recent().getTime();
  const start_date = faker.date.future().getTime();
  const end_date = faker.date.future().getTime();
  ...
  try {
    ...
    for (let i = 0; i < numOfMarketplaceItems; i++) {
      const marketplaceCategory =
        marketplaceItemsArray[i % marketplaceItemsArray.length];
